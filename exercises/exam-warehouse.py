...
# ------------------------------------------
# plan - Returns cost to take all boxes
# in the todo list to dropzone
#
# ----------------------------------------
# modify code below
# ----------------------------------------
def plan(warehouse, dropzone, todo):
    # remove 'x' from the warehouse
    warehouse[dropzone[0]][dropzone[1]] = 0

    cost = 0
    for box in todo:
        box_pos = find_box(warehouse, box)
        cost += 2 * calc_cost(warehouse, dropzone, box_pos)
        warehouse[box_pos[0]][box_pos[1]] = 0

    return cost


def find_box(warehouse, box):
...
def calc_cost(grid, init, goal):
    ...
    while len(open) > 0:
        ...
        for i in range(len(moves)):
            x2 = x + moves[i][0]
            y2 = y + moves[i][1]
            cost2 = cost + moves[i][2]
            if 0 <= x2 < len(grid) and 0 <= y2 < len(grid[0]):
                if grid[x2][y2] == 0 and value[x2][y2] > cost2:
                    value[x2][y2] = cost2
                    open.append([x2, y2])

    # print init, goal, value
