...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    i_expand = [0, 1] + range(4, dim + 2)
    i_take = range(2, dim + 2)

    # process the data
    for k in range(len(data)):

        # expand omega for time t + 1
        Omega = Omega.expand(dim + 2, dim + 2, i_expand, i_expand)
        Xi = Xi.expand(dim + 2, 1, i_expand, [0])
        ...
        # drop measurements for time t
        Omega_ = Omega.take(i_take, i_take)
        Xi_ = Xi.take(i_take, [0])
        A = Omega.take([0, 1], i_take)
        B = Omega.take([0, 1], [0, 1])
        C = Xi.take([0, 1], [0])

        Omega = Omega_ - A.transpose() * B.inverse() * A
        Xi = Xi_ - A.transpose() * B.inverse() * C
