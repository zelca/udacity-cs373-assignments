...
def next_move(hunter_position, hunter_heading, target_measurement, max_distance, OTHER=None):
    ...
    if not OTHER:
        OTHER = [ExtendedKalmanFilter(1.), [target_measurement]]
        xy_estimate = target_measurement
    else:
        ekf = OTHER[0]
        p1 = OTHER[1][-1]
        distance = distance_between(target_measurement, p1)

        # prediction
        Z = matrix([[target_measurement[0], target_measurement[1], distance]])
        x = ekf.localize(Z)
