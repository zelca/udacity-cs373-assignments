...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    # init
    for dim in range(len(Omega)):
        Omega[dim].value[0][0] += 1
        Xi[dim].value[0][0] += 50

    for i in range(N - 1):
        Z, delta = data[i]
        for dim in range(len(Omega)):
            Omega[dim].value[i][i] += 1 / motion_noise
            Omega[dim].value[i][i + 1] += -1 / motion_noise
            Omega[dim].value[i + 1][i] += -1 / motion_noise
            Omega[dim].value[i + 1][i + 1] += 1 / motion_noise
            Xi[dim].value[i][0] += -delta[dim] / motion_noise
            Xi[dim].value[i + 1][0] += delta[dim] / motion_noise
        print Z
        for lm in Z:
            lm_i = N + lm[0]
            for dim in range(len(Omega)):
                Omega[dim].value[i][i] += 1 / measurement_noise
                Omega[dim].value[i][lm_i] += -1 / measurement_noise
                Omega[dim].value[lm_i][i] += -1 / measurement_noise
                Omega[dim].value[lm_i][lm_i] += 1 / measurement_noise
                Xi[dim].value[i][0] += -lm[dim + 1] / measurement_noise
                Xi[dim].value[lm_i][0] += lm[dim + 1] / measurement_noise

    mu_list = []
    for dim in range(len(Omega)):
        mu_list.append(Omega[dim].inverse() * Xi[dim])

    mu_combined = []
    for i in range(mu_list[0].dimx):
        mu_combined.append(mu_list[0].value[i])
        mu_combined.append(mu_list[1].value[i])

    return matrix(mu_combined)  # Make sure you return mu for grading!
